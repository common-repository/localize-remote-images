<?php
	...
	class external_image_replace_get_posts {
		...
		{
			$message = __("Post ID", "localize-remote-images") . " " . filter_input(INPUT_POST, "select_id");
			$error_message = "<span class='error_message'>".__("Error!", "localize-remote-images")."</span> " . $message;
			$select_post = $this->match_post(filter_input(INPUT_POST, "select_id"));
			if(!$select_post) 
			{
				return $error_message . " " . __("is not found.", "localize-remote-images");
			}
			...
			foreach($select_post["img_src"] as $img_src) 
			{
				...
				$file_bin = @file_get_contents($parse_url);

				if(empty($file_bin))
				{
					$flag = FALSE;
				}else{
					file_put_contents($uploaddir_img,$file_bin);
					$flag = file_exists($uploaddir_img);
				}

				if(!$flag) 
				{
					return $error_message . " " . __("Failed to download images.", "localize-remote-images") . " " . $img_src;
				}
				...
				if($attach_id == 0) 
				{
					...
					return $error_message . " " . __("The image could not be registered in the media library.", "localize-remote-images") . " " . $img_src;
				}
				...
				if(!$attach_data) 
				{
					...
					return $error_message . " " . __("Failed to generate thumbnails of images registered in the media library.", "localize-remote-images") . " " . $img_src;
				}
				...
				{
					wp_delete_attachment( $attach_id, true );
					return $error_message . " " . __("Failed to register thumbnails of images registered in the media library.", "localize-remote-images") . " " . $img_src;
				}
				*/
				...
				{
					wp_delete_attachment( $attach_id, true );
					return $error_message . " " . __("Failed to update the article body.", "localize-remote-images") . " " . $img_src;
				}
				$post_content = $post_content_temp;
			}
			return $message . " " . __("Processing is completed.", "localize-remote-images");
		}
	}
